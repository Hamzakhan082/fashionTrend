...
# Lags
agg_data = add_time_step(agg_data)
...
# Encode
agg_data, _ = encode_categoricals(agg_data, ['Category','Season','ProductColor','Gender','AgeGroup'])
...
_, _, split_idx = time_based_split(agg_data, test_size=0.2)
...
X_train, X_test, scaler = scale_numeric_features(X_train, X_test, numeric_feats)
...
for name, model in tuned_models.items():
    ...
    tuned_results[name] = evaluate_predictions(y_test, y_pred)
...
best_after_name = best_model_name(tuned_results)
print(f"Best tuned model: {best_after_name} with R2 = {tuned_results[best_after_name]['R2']:.4f}")
...
best_tuned = tuned_models[best_after_name]
...
top20 = top_predictions(pred_df, n=20)
